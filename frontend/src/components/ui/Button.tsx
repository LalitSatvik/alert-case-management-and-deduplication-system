...
type Variant = "primary" | "highlight" | "default" | "danger" | "ghost";
type Size = "sm" | "md";

const BASE =
  "inline-flex select-none items-center justify-center gap-1.5 rounded-lg border text-sm font-medium " +
  "transition-colors disabled:pointer-events-none disabled:opacity-40 focus-visible:outline-none";

const VARIANT: Record<Variant, string> = {
  primary:
    "border-transparent bg-primary text-primary-fg shadow-xs hover:bg-primary-hover active:bg-primary-active",
  highlight:
    "border-transparent bg-accent text-accent-fg shadow-xs hover:bg-accent-hover active:bg-accent-active",
  default:
    "border-border bg-surface text-ink shadow-xs hover:bg-surface-hover hover:border-border-strong",
  danger:
    "border-danger-border bg-danger-subtle text-danger-subtle-fg hover:border-danger",
  ghost: "border-transparent bg-transparent text-ink-secondary hover:bg-surface-hover hover:text-ink",
};
